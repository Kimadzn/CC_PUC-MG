...
public class FuncionarioDAO extends DAO {
	...
	public List<Funcionario> getsalarioMasculino() {
		List<Funcionario> funcionario = new ArrayList<Funcionario>();
		
		try {
			Statement st = conexao.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
			String sql = "SELECT * FROM funcionario WHERE funcionario.salario";
			System.out.println(sql);
			ResultSet rs = st.executeQuery(sql);	           
	        while(rs.next()) {	            	
	        	Funcionario u = new Funcionario(rs.getInt("id"), rs.getString("nome"), rs.getString("cargo"), rs.getString("salario").charAt(0));
	            funcionario.add(u);
	        }
	        st.close();
		} catch (Exception e) {
